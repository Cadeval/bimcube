...
const PanelLeftCloseIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line x1="9" y1="3" x2="9" y2="21"></line><path d="M15 15l-3-3 3-3"></path></svg>
);
...
export default function LeftPanel() {
  const { 
    ...
    clipping, setClipping, undo, redo, pastInputs, futureInputs, log, loadSession, 
    mainViewMode, setMainViewMode
  } = useStore();
  
  const fileInputRef = useRef(null); 
  const [isMobile, setIsMobile] = useState(window.innerWidth < 768); // 🪄 Mobile detector
  ...
  return (
    <div style={{
      ...
      width: isMobile ? '200px' : '280px', // 🪄 SHRINKS ON MOBILE!
      zIndex: 40, display: 'flex', flexDirection: 'column',
      backgroundColor: isDark ? 'rgba(24, 24, 24, 0.65)' : 'rgba(255, 255, 255, 0.65)', 
      backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', 
      borderRight: `1px solid ${borderCol}`, boxShadow: '4px 0 24px rgba(0,0,0,0.15)',
      transition: 'width 0.3s'
    }}>
      
      <div style={{ 
        display: 'flex', justifyContent: 'space-between', alignItems: 'center', 
        padding: '8px 16px', height: '48px', boxSizing: 'border-box', // 🪄 Shifted Top UP
        borderBottom: `1px solid ${borderCol}` 
      }}>
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          <button onClick={undo} disabled={pastInputs.length === 0} style={{ background: 'transparent', border: 'none', color: pastInputs.length > 0 ? txtCol : dimCol, cursor: pastInputs.length > 0 ? 'pointer' : 'not-allowed', padding: 0 }}>◀</button>
          <button onClick={redo} disabled={futureInputs.length === 0} style={{ background: 'transparent', border: 'none', color: futureInputs.length > 0 ? txtCol : dimCol, cursor: futureInputs.length > 0 ? 'pointer' : 'not-allowed', padding: 0 }}>▶</button>
          ...
          <button onClick={() => fileInputRef.current?.click()} title="Open Session" style={{ background: 'transparent', border: 'none', color: txtCol, cursor: 'pointer', padding: 0 }}>📂</button>
          <button onClick={exportSession} title="Save Session" style={{ background: 'transparent', border: 'none', color: txtCol, cursor: 'pointer', padding: 0 }}>💾</button>
          <button onClick={toggleTheme} title="Toggle Theme" style={{ background: 'transparent', border: 'none', color: txtCol, cursor: 'pointer', display: 'flex', alignItems: 'center', marginLeft: '6px', padding: 0 }}>
             {isDark ? <SunIcon /> : <MoonIcon />} {/* 🪄 Crisp Vectors! */}
          </button>
        </div>
      ...
      <div style={{ padding: '12px 16px 8px 16px', fontWeight: 'bold', textTransform: 'uppercase', fontSize: '0.8rem', letterSpacing: '1px', color: dimCol }}>
        {getTitle()}
      </div>
      ...
      <div style={{ padding: '0 16px 16px 16px', overflowY: 'auto', flex: 1 }}>
        {selectedObject ? (
          <div>
            <button onClick={() => setSelectedObject(null)} style={{ width: '100%', padding: '8px', marginBottom: '16px', background: isDark?'rgba(255,255,255,0.1)':'rgba(0,0,0,0.05)', border: 'none', color: txtCol, cursor: 'pointer', borderRadius: '4px' }}>✕ Deselect</button>
          ...
          <>
                  ...
                  return (
                    <div key={key} style={{ marginBottom: '16px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                        ...
                        <button onClick={() => setMainViewMode(isMapActive ? '3D' : 'Map')} style={{ background: isMapActive ? '#00d1b2' : (isDark ? 'rgba(0,209,178,0.2)' : 'rgba(0,209,178,0.1)'), border: `1px solid #00d1b2`, color: isMapActive ? '#fff' : '#00d1b2', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 'bold', padding: '4px 8px', borderRadius: '4px', transition: 'all 0.2s' }}>
                          {isMapActive ? '🧊 3D View' : '📍 Map View'}
                        </button>
                      </div>
                      <input type="text" value={val} onChange={(e) => setInputValue(key, e.target.value)} style={{ width: '100%', background: isDark ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.5)', color: txtCol, border: `1px solid ${borderCol}`, borderRadius: '4px', padding: '8px', fontSize: '0.85rem', outline: 'none', boxSizing: 'border-box', fontFamily: 'monospace' }} placeholder="Lat, Lng, Rot" />
                    </div>
                  );
                }
            ...
            {/* ... Rest of Layers, Section, Logs remain unchanged ... */}
            {activeTab === 'layers' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <h4 style={{ margin: '0 0 8px 0', color: dimCol, fontSize: '0.7rem', textTransform: 'uppercase' }}>Storeys</h4>
                {availableLevels.map(lvl => (
                  <button key={`lvl-${lvl}`} onClick={() => toggleLevel(lvl)} style={btnStyle(visibility.levels[lvl])} onMouseOver={e=>e.currentTarget.style.background=hoverBg} onMouseOut={e=>e.currentTarget.style.background='transparent'}><span style={{ marginRight: '8px', opacity: visibility.levels[lvl] ? 1 : 0.3 }}>👁️</span> Level 0{lvl}</button>
                ))}
                ...
                {Object.keys(visibility.types).sort().map(type => (
                  <button key={`type-${type}`} onClick={() => toggleType(type)} style={btnStyle(visibility.types[type])} onMouseOver={e=>e.currentTarget.style.background=hoverBg} onMouseOut={e=>e.currentTarget.style.background='transparent'}><span style={{ width: '12px', height: '12px', borderRadius: '3px', backgroundColor: typeColors[type] || '#ccc', marginRight: '10px', opacity: visibility.types[type] ? 1 : 0.2 }}></span>{type}</button>
                ))}
              </div>
            )}
            ...
            {activeTab === 'section' && (
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ display: 'flex', gap: '4px', marginBottom: '24px' }}>
                  {['y', 'x', 'z'].map(axis => (
                    <button key={axis} onClick={() => setClipping({ axis })} style={{ flex: 1, padding: '8px', borderRadius: '4px', border: `1px solid ${borderCol}`, cursor: 'pointer', fontWeight: 'bold', fontSize: '0.8rem', backgroundColor: clipping.axis === axis ? (isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.1)') : 'transparent', color: clipping.axis === axis ? txtCol : dimCol }}>{axis.toUpperCase()}</button>
                  ))}
                </div>
            ...
            {activeTab === 'output' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <button onClick={() => useStore.getState().triggerExport('gltf')} style={{ padding: '10px', backgroundColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)', color: txtCol, border: `1px solid ${borderCol}`, borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: '8px' }} onMouseOver={e=>e.currentTarget.style.backgroundColor=hoverBg} onMouseOut={e=>e.currentTarget.style.backgroundColor=isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)'}><span style={{ fontSize: '1.2rem' }}>📦</span> Export GLB (Recommended)</button>
                <button onClick={() => useStore.getState().triggerExport('obj')} style={{ padding: '10px', backgroundColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)', color: txtCol, border: `1px solid ${borderCol}`, borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: '8px' }} onMouseOver={e=>e.currentTarget.style.backgroundColor=hoverBg} onMouseOut={e=>e.currentTarget.style.backgroundColor=isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)'}><span style={{ fontSize: '1.2rem' }}>🧊</span> Export OBJ (Raw Geometry)</button>
            ...
            {activeTab === 'log' && (
              <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                <button onClick={exportSession} style={{ width: '100%', padding: '10px', marginBottom: '16px', backgroundColor: '#3366ff', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', boxShadow: '0 4px 12px rgba(51, 102, 255, 0.3)' }}>📥 Download Session Log</button>
                <div style={{ flex: 1, color: dimCol, fontSize: '0.75rem', fontFamily: 'monospace', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {log.map((entry, i) => (<div key={i} style={{ paddingBottom: '4px', borderBottom: `1px solid ${borderCol}` }}>{entry}</div>))}
                </div>
              </div>
            )}
          </>
